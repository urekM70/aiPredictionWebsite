...
from tasks.loadData import load_or_fetch_data  # Import the function from your previous script
import os
...
def preprocess_data(df, sequence_length=60):
    ...
    feature_columns = ['Open', 'High', 'Low', 'Close', 'Volume', 'SMA_20', 'SMA_50', 'EMA_12', 'EMA_26', 'RSI', 
                       'MACD', 'MACD_signal', 'ATR', 'Bollinger_Lower', 'Bollinger_Middle', 'Bollinger_Upper', 
                       'Bollinger_Bandwidth', 'Bollinger_Percent', 'Stochastic_K', 'Stochastic_D']
...
def run_crypto_prediction(symbol, interval='1h', start_date=None, end_date=None):
    """Run the crypto prediction model with the given parameters."""
    print("Starting the crypto price prediction process...")

    if start_date is None:
        # Set the default start date to 5 years ago
        start_date = (datetime.datetime.now() - datetime.timedelta(days=5*365)).strftime("%Y-%m-%d")
    
    if end_date is None:
        # Default to today's date as the end date
...
if __name__ == "__main__":
    ...
    if start_date == "":
        start_date = None  # Default to 5 years ago

    end_date = input("Enter end date (YYYY-MM-DD, default is today's date): ")
    if end_date == "":
        end_date = None  # Default to today's date
